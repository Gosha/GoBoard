...
- `goboard-logo.svg` is the editable vector master, using rounded rectangles on a flat dark background. Its keyboard mark uses SteamVR Dashboard blue (`#1A9FFF`), matching `BrandColors.Accent` in the shared renderer. The README uses this SVG directly.
...
```powershell
dotnet run --file assets/branding/render.cs
```
