...
A Windows keyboard overlay for SteamVR, inspired by [YuuBoard](https://yuuzami.itch.io/yuuboard). GoBoard will have its own design and automatically follow the Windows input layout, with Swedish and English support.

The current proof of concept automatically follows the focused Windows application between standard US English (79 keys) and Swedish (80 keys). It includes letters, the number row, punctuation, Space/Backspace/Enter, F1–F12, Esc/Tab, navigation/arrows, click-to-arm/lock Ctrl/Alt/AltGr/Shift, and an arm-then-tap Win key.

## Production project

The maintainable production solution is `GoBoard.slnx`. It preserves the proven POC behavior while separating pure keyboard/grab state, Windows input, Skia presentation, OpenVR hosting, and the executable entry point. Build and test it without a headset:

```powershell
dotnet build GoBoard.slnx -c Release
dotnet test GoBoard.slnx -c Release --no-build
dotnet run --project src/GoBoard.App -c Release -- --render .runtime\panel.png
```

When the POC is stopped, launch the production overlay with `./start-goboard.ps1` and close it with `./stop-goboard.ps1`. The two launch paths use separate runtime files and overlay keys; production startup refuses to create a second typing overlay while the POC is running. See [development and architecture notes](docs/development.md).

The production app now uses the flat midnight-mint design, with 86 US / 87 Swedish buttons, ISO/ANSI geometry, simultaneous primary/Shift/AltGr legends, Caps Lock, Menu, and shared left/right Shift and Ctrl modes. Navigation uses a standard 3-by-2 cluster including Insert, with Print Screen / Scroll Lock / Pause above; Close and Settings are deferred. [Production preview](artifacts/keyboard-implementation/sv-hover.png). To render a Swedish preview without SteamVR, append `--layout sv --state hover` to the render command above. The POC description below documents the earlier interface.

## Run the VR proof of concept
...
Earlier headset feedback confirmed typing and the independent overlay's grab/follow/backface behavior. The previous panels were expanded to 2532x1026 to fit the full number and punctuation rows at the same pixel density and letter-key size. Persistent OpenGL submission previously passed 100 alternating hidden-overlay updates. Blink removal and the expanded panel still need headset confirmation. Native-control history is in [these notes](docs/native-controls-investigation.md).
...
See [the technical design](docs/technical-design.md) for implementation boundaries and acceptance checks.
