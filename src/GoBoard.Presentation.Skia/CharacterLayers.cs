...
internal sealed class CharacterLayers : IDisposable
{
    ...
    private static CharacterLayer Raster(CharacterLabel label, KeyBounds key, SKTypeface face, GRContext context)
    {
        if (label == null) return new(null, null, SKRect.Empty);
        const int scale = OverlayGeometry.RasterScale;
        SKRect bounds = default;
        var image = RenderImage.Create(new SKImageInfo((int)(key.Width * scale), (int)(key.Height * scale),
            SKColorType.Rgba8888, SKAlphaType.Premul), context, canvas =>
        {
