...
{
    private sealed record Track(SKImage From, double Start, double Duration, CharacterTransition Style, int Travel);
    ...
    private static void DrawTrackContent(SKCanvas canvas, Track track, SKImage target, SKRect rect, double now)
    {
        ...
        if (track.Style == CharacterTransition.Lift) { oldRect.Offset(0, -track.Travel * t); newRect.Offset(0, track.Travel * (1 - t)); }
    ...
    public void Draw(SKCanvas canvas, SKImage target, SKImage surfaces, KeyboardState keyboard, KeyboardTheme style, double now, bool filledPress)
    {
        ...
        foreach (var key in current.Keys.Values)
        {
            ...
            var rect = new SKRect(0, 0, b.Width, b.Height);
            // Match Panel's legend clip, including tall punctuation descenders.
            // An inset clip leaves target glyph fragments at the bottom edge.
            canvas.ClipRect(rect);
            const int rasterScale = OverlayGeometry.RasterScale;
            var surfaceCrop = new SKRect(b.X * rasterScale, b.Y * rasterScale, (b.X + b.Width) * rasterScale, (b.Y + b.Height) * rasterScale);
