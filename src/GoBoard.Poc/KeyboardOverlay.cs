...
internal sealed class KeyboardOverlay(CVRSystem system, CVROverlay overlay, ulong handle, OverlayGraphics graphics) : IDisposable
{
    ...
    private bool renderedShift, renderedAltGr, renderedCaps;
    private string renderedStatus, status = "Focus a text field on your desktop";
    private string targetStatus = "Focus a text field on your desktop";
    ...
    public void BeginFrame(bool active, uint? grabbingController = null)
    {
        ...
        if (target != output.Target)
        {
            ...
            targetStatus = State.Layout.Supported ? WindowsKeyboard.TargetName(target) : State.Layout.Status;
    ...
    public void EndFrame()
    {
        ...
        if (renderedRevision == State.Revision && renderedShift == shift && renderedAltGr == altGr && renderedCaps == caps && renderedStatus == status) return;
        using var bitmap = Panel.Render(State, shift, status, altGr, caps);
