...
{
    public const int LayoutWidth = 180, LayoutHeight = 60;
    public const int RasterScale = 3;
    public const int PixelWidth = LayoutWidth * RasterScale, PixelHeight = LayoutHeight * RasterScale;
    public const float Width = 0.18f;
    public static readonly Matrix4x4 PanelOffset = Matrix4x4.CreateTranslation(0, -(Panel.HeightInMeters / 2 + 0.0375f), 0.002f);
    ...
    public static SKBitmap Render(int state)
    {
        ...
        canvas.DrawRoundRect(new SKRect((LayoutWidth - lineWidth) / 2f, (LayoutHeight - lineHeight) / 2f,
            (LayoutWidth + lineWidth) / 2f, (LayoutHeight + lineHeight) / 2f), lineHeight / 2f, lineHeight / 2f, paint);
        canvas.Flush();
        return bitmap;
    }
        ...
        while (overlay.PollNextOverlayEvent(handle, ref e, (uint)Marshal.SizeOf<VREvent_t>()))
        {
            ...
            switch (type)
            {
                case EVREventType.VREvent_FocusEnter:
                    ...
                    Console.WriteLine($"Grab focus entered: cursor {e.data.overlay.cursorIndex}, device {device?.ToString() ?? "unknown"}, raw device {e.trackedDeviceIndex}, path {e.data.overlay.devicePath}.");
                    break;
                case EVREventType.VREvent_FocusLeave:
                    input.Leave(pointer.Value, device, time);
                    Console.WriteLine($"Grab focus left: cursor {e.data.overlay.cursorIndex}, device {device?.ToString() ?? "unknown"}.");
                    break;
                case EVREventType.VREvent_MouseButtonDown when e.data.mouse.button == (uint)EVRMouseButton.Left:
                    ...
                    Console.WriteLine($"Grab down: cursor {e.data.mouse.cursorIndex}, raw device {e.trackedDeviceIndex}, age {e.eventAgeSeconds:F3}s, point {e.data.mouse.x:F1},{e.data.mouse.y:F1}; {rejection ?? "pending target validation"}.");
                    break;
                case EVREventType.VREvent_MouseButtonUp when e.data.mouse.button == (uint)EVRMouseButton.Left:
                    ...
                    Console.WriteLine($"Grab up: cursor {e.data.mouse.cursorIndex}, raw device {e.trackedDeviceIndex}, age {e.eventAgeSeconds:F3}s.");
                    break;
    ...
    private bool TryPose(uint device, out Matrix4x4 pose)
    {
        ...
        return IsController(device) && devices[device].bDeviceIsConnected && devices[device].bPoseIsValid &&
            RelativePose.TryRigid(devices[device].mDeviceToAbsoluteTracking, out pose);
    }
