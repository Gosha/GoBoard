...
internal static class KeyboardChecks
{
    ...
    public static void Verify()
    {
        ...
        (float X, float Y) Point(string id) { var b = Key(id).Bounds; return (b.X + b.Width / 2, Panel.LayoutHeight - b.Y - b.Height / 2); }
        ...
        Require(KeyboardLayout.Keys.Count == 79 && KeyboardLayout.Keys.Select(k => k.Id).Distinct().Count() == 79, "Keyboard layout lost or duplicated a key.");
        Require(KeyboardLayout.Keys.All(k => k.Id != "Start") && KeyboardLayout.Keys.Select(k => k.Scan).Distinct().Count() == 79, "Start key remains or physical key is duplicated.");
        foreach (var k in KeyboardLayout.Keys)
        {
            var p = Point(k.Id);
            Require(KeyboardLayout.HitOpenVr(p.X, p.Y) == k, $"Pointer orientation misses {k.Id}.");
        }
        Require(KeyboardLayout.Hit(61, 105) == null && KeyboardLayout.Hit(float.NaN, 60) == null, "Gap or invalid coordinate hit a key.");
        ...
        Require(state.Layout.Keys.Count == 80 && state.Layout.Keys.All(k => k.Id != "Start") && state.Layout.Keys.Single(k => k.Id == "Iso").Scan == 0x56, "Swedish ISO geometry or Start removal incorrect.");
        ...
        Console.WriteLine("Keyboard checks passed: 79 US/80 Swedish keys, Win arm/tap/re-arm and failure cleanup, ISO geometry, AltGr chords, layout-change cancellation, unsupported-layout guard, shortcuts, repeat and pointer ownership.");
    }
}
