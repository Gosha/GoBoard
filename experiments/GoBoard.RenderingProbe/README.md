...
The format probes use the live main texture's current dimensions (3138×846), but stretch the base keyboard across that surface. They do **not** reproduce the live centered content bounds, numpad, expanded shortcuts, dual pointers, or real scheduling. Use them only as a format sensitivity check. Geometry constants must be revisited if the live texture dimensions change. Their existing benchmark header says "Excludes presentation/upload" because no GPU upload is performed.
