...
// Mirrors Panel's printable-legend placement for Steam Flat. Actual settled
...
internal sealed class CharacterLayers : IDisposable
{
    ...
    public static CharacterLayers Capture(LocalSession session)
    {
        var result = new CharacterLayers(); var layout = session.Keyboard.Layout;
        using var face = SKTypeface.FromFamilyName("Segoe UI", SKFontStyle.Normal);
        using var letter = new SKFont(face, 21);
        var style = KeyboardTheme.Flat;
        foreach (var key in layout.Keys.Where(k => k.Printable))
        {
            ...
            var altGr = session.Keyboard.AltGr && layout.HasAltGr;
            var active = layout.Legend(key, session.Keyboard.Shift, altGr, session.Caps);
            ...
            var upper = session.Keyboard.Shift && !altGr ? normal.Text : shifted.Text;
