...
The workflow reports four checks: version rules/release metadata, build/regression tests, Stable MSI, and Rolling MSI. Version checks and regression tests start independently. Once versions are resolved, both MSI jobs run in parallel with each other and any still-running tests. An MSI failure does not cancel the other channel; each successful job uploads its artifacts immediately. Publishing waits for all four checks to succeed. Test runs emit TRX files uploaded as the msi-test-results artifact, including when tests fail; earlier failures that produce no results do not cause an additional upload failure. Only the selected channel is published on a push. A Rolling build is kept as a workflow artifact without publication if `main` has already advanced by the publication check.
