...
The Release build (zero warnings/errors), all 354 regressions after rebasing onto the numpad-placement changes, native GPU comparison, actual SteamVR publication/failure fallback, and the 13-update numpad and 81-update shortcut resize checks passed. Animated and settled VR previews plus the desktop preview were visually inspected. The transparent integration checks also observed cold-start spikes in drawing/publication, so warm medians should not be applied to startup.
